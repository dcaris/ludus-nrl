...
class NrlCompetition extends Competition {
    ...
    async results(round, detailed = false) {
        ...
        if (response.status == 200) {
            ...
            // Parse out each game
            const allGames = games.map(async function(g) {
                if (detailed) {
                    // /draw/nrl-premiership/2018/round-17/panthers-vs-warriors/
            ...
            matches = await Promise.all(allGames);
        }
        return matches;
    }
    ...
    async match(year, round, homeTeamNickname, awayTeamNickname) {
        ...
        if (response.status == 200) {
            ...
            // Collect the main match details
            var matchDate = moment(json.match.startTime);
            var match = new Match(matchDate.toDate(), json.match.venue, json.match.attendance);
            match.id = json.match.matchId;

            //match.mode = Post; // TODO: handle future not completed games

            // Build away team details
            match.awayTeam = new Team(new Club(json.match.awayTeam.nickName)); // TODO: club id extraction
            match.awayTeam.score = json.match.awayTeam.score;
            match.awayTeam.players = NrlCompetition.getTeamStats(json.match.awayTeam,
                json.match.stats.players.awayTeam);

            // Build home team details
            match.homeTeam = new Team(new Club(json.match.homeTeam.nickName)); // TODO: club id extraction
            match.homeTeam.score = json.match.homeTeam.score;
