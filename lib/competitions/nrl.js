            ...
            axios.default.get(`${self.url}/draw/nrl-premiership/${round.year}/round-${round.number}`)
                .then((response) => {
                    if (response.status == 200) {
                        ...
                        var filterData = $('div[id="vue-draw"]').attr('q-data').trim();
                        var json = JSON.parse(filterData);
                        json.drawGroups.forEach(function(i) {
                            var matchDateText = `${i.title} ${round.year}`;
                            var matchDate = moment(matchDateText, "dddd Do MMMM YYYY");
                            var match = new Match(matchDate.toDate());
                            matches.push(match);
                        });
                        resolve(matches);
                    }
